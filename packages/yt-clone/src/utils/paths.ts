...
export type BuildPathArgs<Path extends RoutePath> = keyof RouteParams<Path> extends never
  ? {
      path: Path
...
export const buildPath = <Path extends RoutePath>({ params, path, query }: BuildPathArgs<Path>) => {
